...
def _action_queue_items(db: Session) -> list[dict]:
    ...
    for request in db.scalars(
        select(ApprovalRequest).where(
            ...
            ApprovalRequest.resource_type == "negotiation_adjustment",
        )
    ).all():
        ...
        items.append(
            {
                ...
                "type": "negotiation_adjustment",
                "campaign_id": request.campaign_id,
                "campaign_name": campaign.name,
                "title": "Review creator counteroffer",
                "detail": "One bounded rate adjustment is available.",
                "amount_cents": int(request.context.get("requested_rate_cents") or 0),
                "expected_version": campaign.version,
    ...
    return {
        ...
        "services": [
            ...
            },
            {
                ...
                if not (
                    db.scalar(
                        select(func.count(HermesTask.id)).where(
                            HermesTask.status == "failed"
                        )
                    )
                    or 0
                )
...
@v1.get("/system/acceptance-proof")
async def acceptance_proof(
    campaign_id: str | None = None,
    db: Session = Depends(get_db),
    settings: Settings = Depends(settings_dep),
    providers: Providers = Depends(providers_dep),
    ...
    messages = db.scalars(
        select(DealMessage).join(Deal).where(Deal.campaign_id == campaign.id)
    ).all()
    qa_checks = db.scalars(
        select(QACheck).join(Deliverable).join(Deal).where(Deal.campaign_id == campaign.id)
    ).all()
    closed_event = db.scalar(
        select(DomainEvent).where(
            DomainEvent.campaign_id == campaign.id,
            DomainEvent.event_type == "campaign_run_closed",
        )
    )
    learned_next = None
    if learning and learning.skill_version:
        learned_next = db.scalar(
            select(CampaignStrategy)
            .join(Campaign)
            .where(
                Campaign.created_at >= campaign.created_at,
                CampaignStrategy.skill_version >= learning.skill_version,
    ...
    criteria = [
        {
            "id": 1,
            "name": "Hermes in NemoClaw on Nemotron 3 Ultra",
            "passed": (
                settings.capability_configured("hermes")
                and settings.require_nemoclaw
                and settings.hermes_model == "nvidia/nemotron-3-ultra-550b-a55b"
                and providers.hermes.healthy()
            ),
            "evidence": {
        ...
        {
            "id": 2,
            "name": "Hermes strategy uses a skill version",
            "passed": bool(campaign.strategy and campaign.strategy.skill_version),
            "evidence": {
        ...
        {
            "id": 3,
            "name": "Brand funding settled through Stripe",
            "passed": bool(
                funding
                and funding.status == "succeeded"
                and funding.payment_intent_id
                and funding.source_charge_id
            ),
            "evidence": {
        ...
        {
            "id": 4,
            "name": "Hermes provisioned live creator discovery",
            "passed": bool(campaign.deals)
            and all(deal.creator.email for deal in campaign.deals),
            "evidence": {
                "creators": len(campaign.deals),
                "verified_contacts": sum(bool(deal.creator.email) for deal in campaign.deals),
            },
        },
        {
            "id": 5,
            "name": "Hermes discovered and negotiated with a creator",
            "passed": bool(
                campaign.deals
        ...
        {
            "id": 8,
            "name": "Campaign closure automatically started learning",
            "passed": bool(closed_event and learning),
            "evidence": {
                "run_id": campaign.run_id,
                "learning_status": learning.status if learning else None,
            },
        },
        {
            "id": 9,
            "name": "Priors, reputation, and optional skill version changed",
            "passed": bool(
                learning
                and learning.baseline_status == "applied"
                and learning.database_updates
                and (
                    campaign.learning_mode == "database"
                    or learning.patch_status in {"applied", "no_op"}
                )
            ),
            "evidence": learning_projection(learning, campaign) if learning else None,
        },
        {
            "id": 10,
            "name": "A later strategy visibly uses learned evidence",
            "passed": bool(
                learned_next
                or (
                    campaign.learning_mode == "database"
                    and learning
                    and learning.database_updates.get("strategy_prior")
                )
            ),
            "evidence": {
                "later_strategy_id": learned_next.id if learned_next else None,
                "later_skill_version": learned_next.skill_version if learned_next else None,
                "database_prior_available": bool(
                    learning and learning.database_updates.get("strategy_prior")
                ),
            },
        },
    ]
...
async def hermes_tasks_preflight(db: Session = Depends(get_db)) -> dict:
    ...
    failed = (
        db.scalar(select(func.count(HermesTask.id)).where(HermesTask.status == "failed")) or 0
    )
    return {
        "pending": pending,
        "claimed": claimed,
...
@v1.post("/deals/{deal_id}/creator-reply")
async def deals_creator_reply(
    deal_id: str,
    data: CreatorReplyCreate,
    db: Session = Depends(get_db),
    providers: Providers = Depends(providers_dep),
) -> dict:
    return process_creator_reply(db, must_get(db, Deal, deal_id), data, providers)
