...
def _approval_text(db: Session, request: ApprovalRequest) -> str:
    ...
    detail = ""
    if request.resource_type == "negotiation_adjustment":
        detail = (
            f"\nCurrent ${int(context.get('current_rate_cents', 0)) / 100:.2f}; "
            f"creator requested ${int(context.get('requested_rate_cents') or 0) / 100:.2f}."
        )
