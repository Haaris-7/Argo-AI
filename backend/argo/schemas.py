...
class CompensationPlan(BaseModel):
    pricing_mode: Literal["user", "argo"] = "user"
...
class NegotiationPolicy(BaseModel):
    ...
    max_escalation_percent: int = Field(default=10, ge=0, le=10)
    persuasion_attempts: int = Field(default=2, ge=2, le=2)
    replacement_limit: int = Field(default=3, ge=0, le=3)
...
class ApprovalCreate(BaseModel):
    campaign_id: str
    resource_type: Literal["strategy", "deal", "service_spend", "payout", "negotiation_adjustment"]
    resource_id: str
...
class CreatorReplyCreate(BaseModel):
...
class LearningCommit(BaseModel):
    ...
    skill_name: str = Field(pattern=r"^argo-[a-z0-9-]+$")
...
class HermesLearning(BaseModel):
    ...
    skill_name: str = "argo-performance-learning"
    evidence_ids: list[str]
    governance: dict[str, Any] = Field(default_factory=dict)


class HermesNegotiation(BaseModel):
    intent: Literal["accept", "counter", "decline"]
    response: str
    agreed_rate_cents: int | None = Field(default=None, ge=1)
