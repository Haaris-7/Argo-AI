...
def create_campaign(db: Session, data: CampaignCreate) -> Campaign:
    ...
    payload = data.model_dump(exclude={"compensation", "negotiation_policy"})
    compensation = data.compensation.model_dump() if data.compensation else {}
    source = (
        "user"
        if data.compensation
        else ("legacy" if "payout_model" in data.model_fields_set else "argo")
    )
    campaign = Campaign(
        **payload,
        compensation=compensation,
        compensation_source=source,
        negotiation_policy=data.negotiation_policy.model_dump(),
        qa_mode="two_stage",
    )
    db.add(campaign)
    db.flush()
...
def generate_strategy(db: Session, campaign: Campaign, providers: Providers) -> CampaignStrategy:
    ...
    latest_skill_record = db.scalar(
        select(SkillVersion)
        .where(
            SkillVersion.skill_name == "argo-strategy-engine",
    ...
    if not (campaign.compensation or {}).get("components"):
        proposed = (
            ...
            if campaign.compensation_source == "argo" and recommendation.compensation_components
            else _legacy_components(campaign.payout_model, recommendation.target_rate_cents)
        )
        campaign.compensation = {
            "pricing_mode": "argo" if campaign.compensation_source == "argo" else "user",
            "components": proposed,
        }
...
def mark_funded(
    ...
    if funding.status != "succeeded":
        ...
        enqueue_hermes_task(
            db, campaign_id, "launch", dedupe_key=f"launch:{campaign_id}"
        )
        db.commit()
    return funding
...
def close_and_replace_deal(
    ...
    *,
    ...
    limit = int((deal.campaign.negotiation_policy or {}).get("replacement_limit", 3))
...
def decide_approval(db: Session, request: ApprovalCreate, *, stripe_live: bool) -> Approval:
    ...
    if request.resource_type == "strategy":
        ...
        if request.decision == "approved":
            ...
            transition_campaign(db, campaign, CampaignStatus.AWAITING_FUNDING.value)
            enqueue_hermes_task(
                db, campaign.id, "funding", dedupe_key=f"funding:{campaign.id}"
            )
        else:
    ...
    elif request.resource_type == "negotiation_adjustment":
        deal = must_get(db, Deal, request.resource_id)
        if deal.campaign_id != campaign.id or deal.negotiation_escalation_status != "pending":
            raise HTTPException(status_code=409, detail="Negotiation is not awaiting escalation")
        if request.decision == "approved":
            current = deal.agreed_rate_cents or campaign.strategy.target_rate_cents
            ceiling = round(
                current
                * (
                    1
                    + int((campaign.negotiation_policy or {}).get("max_escalation_percent", 10))
                    / 100
                )
            )
            revised = min(
                deal.negotiation_requested_rate_cents or ceiling,
                ceiling,
                campaign.per_creator_cap_cents,
            )
            deal.agreed_rate_cents = revised
            components = list((deal.compensation or {}).get("components", []))
            primary_kind = (
                "base"
                if any(c.get("kind") == "base" for c in components)
                else (components[0].get("kind") if components else "base")
            )
            deal.compensation = {
                **(deal.compensation or {}),
                "components": [
                    {**component, "rate_cents": revised}
                    if component.get("kind") == primary_kind
                    else component
                    for component in components
                ],
            }
            deal.negotiation_escalation_status = "approved"
        else:
            deal.negotiation_escalation_status = "rejected"
...
def send_outreach(
    ...
    if deal.campaign.operation_mode == "full_autonomy" and (
        deal.campaign.compensation_source == "argo"
    ):
...
def process_creator_reply(
    db: Session,
    deal: Deal,
    data: CreatorReplyCreate,
    providers: Providers,
) -> dict[str, Any]:
    if deal.status not in {DealStatus.CONTACTED.value, DealStatus.NEGOTIATING.value}:
        raise HTTPException(status_code=409, detail="Deal is not open for negotiation")
    existing = db.scalar(
    ...
    amounts = re.findall(r"\$\s*([0-9]+(?:\.[0-9]{1,2})?)", data.body)
    requested_rate = round(float(amounts[0]) * 100) if amounts else None
    inbound = DealMessage(
        deal_id=deal.id,
        direction="inbound",
        ...
        body=data.body,
        proposed_rate_cents=requested_rate,
    )
    db.add(inbound)
    db.flush()
    current_rate = deal.agreed_rate_cents or _primary_rate(
        list((deal.compensation or {}).get("components", [])),
        deal.campaign.strategy.target_rate_cents,
    )
    lowered = data.body.lower()
    explicit_decline = any(word in lowered for word in ("no thanks", "decline", "not interested"))
    accepted = not explicit_decline and (
        (requested_rate is not None and requested_rate <= current_rate)
        or any(word in lowered for word in ("agree", "accepted", "works for me", "yes"))
    )
    close_reason = None
    if explicit_decline:
        intent = "decline"
        response_text = "Thanks for considering the campaign. We will close this opportunity."
        close_reason = "creator_declined"
    elif accepted:
        intent = "accept"
        agreed = min(requested_rate or current_rate, current_rate)
        deal.agreed_rate_cents = agreed
        components = list((deal.compensation or {}).get("components", []))
        primary_kind = (
            "base"
            if any(c.get("kind") == "base" for c in components)
        ...
        response_text = (
            f"Confirmed at ${agreed / 100:.2f}. Your email acceptance is recorded as the "
            "agreement. Reply with DRAFT: followed by a public media URL and include the "
            "caption in the same email. We will run NVIDIA QA and reply in this thread."
        )
    else:
        deal.negotiation_pushes += 1
        deal.negotiation_requested_rate_cents = requested_rate
        policy = deal.campaign.negotiation_policy or {}
        persuasion_attempts = int(policy.get("persuasion_attempts", 2))
        escalation_status = deal.negotiation_escalation_status
        if (
            deal.campaign.operation_mode == "full_autonomy"
            and deal.negotiation_pushes > persuasion_attempts
        ):
            intent = "decline"
            response_text = (
                "We cannot move beyond the approved market rate, so we will close this opportunity."
            )
            close_reason = "autonomous_offer_refused"
        elif (
            escalation_status in {"approved", "rejected"}
            and deal.negotiation_pushes > persuasion_attempts
        ):
            intent = "decline"
            response_text = (
                "We cannot revise the final approved terms further, so we will close "
                "this opportunity."
            )
            close_reason = "final_offer_refused"
        elif deal.negotiation_pushes > persuasion_attempts and policy.get(
            "escalation_enabled", True
        ):
            intent = "counter"
            deal.negotiation_escalation_status = "pending"
            create_approval_request(
                db,
                deal.campaign_id,
                "negotiation_adjustment",
                deal.id,
                {"requested_rate_cents": requested_rate, "current_rate_cents": current_rate},
            )
            response_text = (
                "Your counter is outside the approved terms. We have requested one final "
                "operator review."
            )
        else:
            intent = "counter"
            response_text = (
                f"Our approved terms remain ${current_rate / 100:.2f}. "
                "The campaign is QA-gated and includes the agreed performance structure."
            )
        deal.status = DealStatus.NEGOTIATING.value
    ...
    if accepted:
        ...
        close_and_replace_deal(db, deal, providers, reason=close_reason)
    else:
        db.commit()
    return {
        "deal_id": deal.id,
...
def process_creator_email_updates(db: Session, providers: Providers) -> int:
    """Poll Gmail threads and advance negotiation/deliverable state idempotently."""
    open_statuses = {
    ...
    for reply in providers.mail.thread_messages(set(by_thread)):
        ...
        if db.scalar(
            ...
            continue
        if deal.status in {DealStatus.CONTACTED.value, DealStatus.NEGOTIATING.value}:
            process_creator_reply(
                db,
                deal,
                CreatorReplyCreate(
                    body=reply.body,
...
def request_payout(
    ...
    if (
        ...
        enqueue_hermes_task(
            db, campaign.id, "learning", dedupe_key=f"learning:{campaign.id}"
        )
    db.commit()
        ...
        "negotiations": [
            {
                "deal_id": deal.id,
                "initial_rate_cents": (
                    campaign.strategy.target_rate_cents if campaign.strategy else None
                ),
                "agreed_rate_cents": deal.agreed_rate_cents,
                "result": deal.status,
                "rounds": sum(message.direction == "inbound" for message in deal.messages),
            }
            for deal in campaign.deals
        ],
        "experiments": [
    ...
    try:
        if learning.baseline_status != "applied":
            ...
            for deal in campaign.deals:
                ...
                target_rate = campaign.strategy.target_rate_cents if campaign.strategy else 0
                negotiation_observation = (
                    75
                    if deal.agreed_rate_cents
                    and target_rate
                    and deal.agreed_rate_cents <= target_rate
                    else 55
                    if deal.agreed_rate_cents
                    else 35
                )
                reputation.negotiation = (
                    reputation.negotiation * weight + negotiation_observation
                ) / (weight + 1)
                reputation.observations += 1
                reputation.overall = (
                    reputation.performance * 0.4
                    + reputation.reliability * 0.3
                    + reputation.negotiation * 0.15
                    + reputation.audience_quality * 0.15
                )
...
def _reputation_snapshot(reputation: CreatorReputation) -> dict[str, float | int]:
    return {
        ...
        "negotiation": round(reputation.negotiation, 2),
...
def campaign_state(db: Session, campaign: Campaign) -> dict[str, Any]:
    ...
    return {
        "campaign": {
            ...
            "negotiation_policy": campaign.negotiation_policy,
        ...
        "deals": [
            {
                ...
                "negotiation_pushes": deal.negotiation_pushes,
                "negotiation_escalation_status": deal.negotiation_escalation_status,
