...
def register(ctx) -> None:
    specs = [
        (
            "argo_get_campaign",
            "Read the authoritative state for an Argo campaign.",
            _with_task_id({"campaign_id": {"type": "string"}}),
            ["campaign_id"],
            tools.argo_get_campaign,
        ),
        (
            "argo_request_discovery",
            "Request policy-metered creator discovery for a campaign.",
            _with_task_id({"campaign_id": {"type": "string"}}),
            ["campaign_id"],
            tools.argo_request_discovery,
        ),
        (
            "argo_request_service_spend",
            (
            ...
            ["campaign_id"],
            tools.argo_request_service_spend,
        ),
        (
            "argo_request_outreach",
        ...
        (
            ...
            ["deliverable_id"],
            tools.argo_request_qa,
        ),
        (
            "argo_process_creator_reply",
            "Let Hermes classify and answer an inbound creator negotiation within policy caps.",
            _with_task_id(
                {
                    "deal_id": {"type": "string"},
                    "body": {"type": "string"},
            ...
            tools.argo_process_creator_reply,
        ),
        (
            "argo_request_payout",
            ...
            ["payout_id"],
            tools.argo_request_payout,
        ),
        (
            "argo_notify_operator",
            ...
            ["text", "dedupe_key"],
            tools.argo_notify_operator,
        ),
        (
            "argo_begin_learning",
            "Snapshot Argo skill versions before a campaign learning cycle.",
            _with_task_id({"run_id": {"type": "string"}}),
            ["run_id"],
            tools.argo_begin_learning,
        ),
        (
            "argo_commit_learning",
            "Record the evidence-backed result after updating an argo-* skill.",
            _with_task_id(
                {
                    "run_id": {"type": "string"},
                    "summary": {"type": "string"},
                    ...
                    "skill_name": {"type": "string", "pattern": "^argo-[a-z0-9-]+$"},
                    "evidence_ids": {"type": "array", "items": {"type": "string"}},
                    "governance": {"type": "object"},
                }
            ),
            ["run_id", "summary", "skill_name", "evidence_ids"],
            tools.argo_commit_learning,
        ),
        (
            ...
            _with_task_id({"campaign_id": {"type": "string"}}),
            ["campaign_id"],
            tools.argo_generate_strategy,
        ),
        (
            "argo_launch_campaign",
            "Launch a funded campaign into active discovery and outreach.",
            _with_task_id({"campaign_id": {"type": "string"}}),
            ["campaign_id"],
            tools.argo_launch_campaign,
        ),
        (
            "argo_create_funding",
        ...
        (
            ...
            _with_task_id({}),
            [],
            tools.argo_preflight,
        ),
        (
            "argo_claim_tasks",
            "Claim pending Hermes work tasks with a 180-second lease for execution.",
            _with_task_id({"limit": {"type": "integer", "minimum": 1, "maximum": 10, "default": 5}}),
            [],
            tools.argo_claim_tasks,
        ),
        (
            "argo_complete_task",
