...
Read the campaign through `argo_get_campaign` first to understand context.
...
After strategy generation, use `argo_complete_task` if this was a Hermes cron task,
or `argo_fail_task` if the generation raised an error.
