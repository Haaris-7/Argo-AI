#!/usr/bin/env bash
#
# Run the Argo backend (and frontend, if Node is available) locally for development.
...
.venv/bin/python -m uvicorn argo.main:app --app-dir backend --host 127.0.0.1 --port 8000 --reload &
pids+=($!)
